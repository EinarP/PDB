...
TODO: LM not best for fat%, but could be useful for correlation with weight
...
act <- act[ ,c("date","steps","trnaer","trnana","socact","homework")]
...
homework <- aggregate(homework ~ yearweek, data=act, length)
actsum <- Reduce(function(x, y) merge(x, y, all=TRUE), list(stepsmean, trnana, trnaer, socact, homework))

tail(actsum, 8, row.names=FALSE, row.names=FALSE)

actsum[is.na(actsum)] <- 0
barplot(t(as.matrix(actsum[,2:6])), names.arg=actsum[,1], col=c("darkgreen", "darkred", "darkblue", "yellow","white"), ylim=c(0, 10), legend.text=T, args.legend=list(ncol=4, x="topleft"))
box()
abline(h=3, lty=3)
abline(h=6, lty=3)
```
...
TODO: According Rhonda Patrick there is optimal range for vitamin D, too much is harmful
...
# Optimal level below 2.5 by [TRANCEND]
abline(h=2.5, col=goalCol) 
...
TODO! Color coding for optimal and not optimal values (to indicate if desired value top or below red line)
